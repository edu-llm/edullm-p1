...
def test_trainer_binds_edullm_data_not_legacy_bucket():
    ...
    pacing = (_TRAIN.parent / "curriculum_pacing.py").read_text(encoding="utf-8")
    assert 'CURRICULUM_DATASET_ID = "curriculum/regmix-370m"' in pacing
    assert "curriculum_order_group" in text
    assert "CURRICULUM_DATASET_ID" in text
    assert "curriculum/regmix-compression-370m" not in text


def test_trainer_s3_is_input_only_and_artifacts_use_wandb():
    text = _TRAIN.read_text(encoding="utf-8")
    assert "publish_completed_checkpoint" in text
    assert "wandb_log_runtime_artifacts" in text
    assert "_wandb_log_artifact_and_wait" in text
    assert "_abort_all_ranks" in text
    assert "stage_load_path" in text
    assert "ephemeral_runtime" in text
    # Fail-closed checkpoint publication (not warn-only or finish-only).
    assert "W&B checkpoint publication failed" in text
    assert "wait()" in text
    assert "fail closed via broadcast" in text or "_abort_all_ranks" in text
    # S3 remains a read-only source for published training inputs, never an
    # artifact/checkpoint destination.
    assert 'DATA_BUCKET = "edullm-data"' in text
    assert "stage_edullm_uris" in text
    assert "sync_to_s3" not in text
    assert "sync_from_s3" not in text
    assert "publish_last_durable_step" not in text
    ...
    # Scratch layout includes metrics/, uploaded in the W&B runtime artifact.
    assert '(Path(metrics_dir), "metrics")' in text
...
def test_trainer_wandb_smollm_protocol():
    ...
    assert "def wandb_log_runtime_artifacts" in text
    assert "def wandb_drain_task_loss_evals" in text
    assert "last_wandb_step.json" in text
