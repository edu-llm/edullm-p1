...
#   datasets/farmshare/push_aws_session_to_farmshare.sh /scratch/users/$SUNET/refhq-new-v1
set -Eeuo pipefail
...
ssh -S "${SOCK}" -o BatchMode=yes "${HOST}" \
  "mkdir -p ${STAGING}/datasets/refhq_new/scripts ${STAGING}/datasets ${STAGING}/datasets/farmshare"
...
rsync -avz -e "ssh -S ${SOCK} -o BatchMode=yes" \
  ...
  "${REPO_ROOT}/datasets/farmshare/push_aws_session_to_farmshare.sh" \
...
SUNET=${SUNET}
RUN_NAME=${RUN_NAME}
mkdir -p "\${RUN_DIR}/datasets" "\${RUN_DIR}/datasets/farmshare" "\${RUN_DIR}/logs"
...
echo "next: on laptop, datasets/farmshare/push_aws_session_to_farmshare.sh \${RUN_DIR}"
REMOTE
