...
# Published edullm-data token-order dataset: one dataset id, one group per metric.
CURRICULUM_DATASET_ID = "curriculum/regmix-370m"

# Group names under CURRICULUM_DATASET_ID (first path segment in publish layout).
CURRICULUM_ORDER_GROUP_FOR_METRIC: dict[str, str] = {
...
def curriculum_order_group(metric: str) -> str:
    """Return the edullm-data group for a difficulty metric."""
    try:
        return CURRICULUM_ORDER_GROUP_FOR_METRIC[metric]
    except KeyError as e:
        raise ValueError(
            f"unknown difficulty metric {metric!r}; "
            f"expected one of {sorted(CURRICULUM_ORDER_GROUP_FOR_METRIC)}"
        ) from e
