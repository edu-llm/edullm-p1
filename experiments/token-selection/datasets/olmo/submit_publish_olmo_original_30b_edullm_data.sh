#!/usr/bin/env bash
# Stage olmo30b (~31B measured) on FarmShare → edullm-landing → edullm-data.
# Dataset: pretrain/olmo-original-30b
set -Eeuo pipefail
...
sed -i 's/self\.out_dir = out_di$/self.out_dir = out_dir/' \
  "${RUN_DIR}/scripts/olmo/publish_olmohq_edullm_data.py"
...
echo "target_dataset=pretrain/olmo-original-30b"
echo "constraints: source-only labels; max 1GiB shards; val=0.15% per source"
echo "io: FarmShare Slurm node streams from s3://edullm-datasets then publishes to landing"
