#!/usr/bin/env python3
"""Self-contained synchronous 20-label OLMES evaluator for OLMo2-370M."""
...
LOG = logging.getLogger("token_selection_task_loss")
TASK_LABELS = (
...
class EvaluatorContractError(RuntimeError):
    """The checkpoint or evaluator does not satisfy the locked suite."""
...
def require_suite() -> None:
    ...
    if missing:
        raise EvaluatorContractError(
            f"installed ai2-olmo is missing {len(missing)} locked labels; first={missing[0]!r}"
        )
...
def materialize_model_eval(checkpoint: Path) -> Path:
    ...
    temporary = Path(tempfile.mkdtemp(prefix="token-selection-unshard-"))
    try:
        unshard_checkpoint(
            dir=str(source),
...
def load_model(checkpoint: Path, model: torch.nn.Module) -> int:
    ...
    if not isinstance(payload, dict) or not isinstance(payload.get("model"), dict):
        raise EvaluatorContractError(f"invalid evaluator payload: {path}")
    embedding = payload["model"].get("embeddings.weight")
...
def evaluate_label(
    ...
    evaluator = build_label_evaluator(config, tokenizer, device, label, device_eval_batch_size)
...
def warm_inputs(
    ...
    if dist.get_rank() == 0:
        for label in TASK_LABELS:
            evaluator = build_label_evaluator(
                config, tokenizer, device, label, device_eval_batch_size
            )
            try:
